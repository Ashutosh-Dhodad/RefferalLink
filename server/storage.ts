...
export class DatabaseStorage implements IStorage {
  ...
  async getUser(id: string): Promise<User | undefined> {
    ...
    const user = Array.isArray(usersResult) ? usersResult[0] : undefined;
  ...
  async getProfile(userId: string): Promise<Profile | undefined> {
    const profilesResult = await db.select().from(profiles).where(eq(profiles.userId, userId));
    const profile = Array.isArray(profilesResult) ? profilesResult[0] : undefined;
  ...
  async getCompany(id: string): Promise<Company | undefined> {
    const companiesResult = await db.select().from(companies).where(eq(companies.id, id));
    const company = Array.isArray(companiesResult) ? companiesResult[0] : undefined;
  ...
  async getJob(id: string): Promise<Job | undefined> {
    const jobsResult = await db.select().from(jobs).where(eq(jobs.id, id));
    const job = Array.isArray(jobsResult) ? jobsResult[0] : undefined;
  ...
  async getJobs (filters: {
    ...
    return filteredJobs
      .map(job => ({
        ...job,
        workStyle: (["hybrid", "remote", "onsite"].includes(job.workStyle) ? job.workStyle : null) as "hybrid" | "remote" | "onsite" | null,
        status: (
          job.status === "draft"
            ? "paused"
            : (["active", "closed", "paused"].includes(job.status) ? job.status : null)
        ) as "active" | "closed" | "paused" | null,
  ...
  async getApplication(id: string): Promise<Application | undefined> {
    const applicationsResult = await db.select().from(applications).where(eq(applications.id, id));
    const application = Array.isArray(applicationsResult) ? applicationsResult[0] : undefined;
  ...
  async getApplicationsByCompany(companyId: string): Promise<Application[]> {
    ...
    const jobIds = Array.isArray(companyJobs) ? companyJobs.map(job => job.id) : [];
